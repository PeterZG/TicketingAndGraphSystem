...
public class GraphTest {
    ...
    public Graph<Integer> setupGraphOfIntegers() {
     ...
     public void testBFSStrings() {
        ...
        Iterator<String> iter1 = graph.iterator();
        assertEquals("A", iter1.next());
        assertEquals("B", iter1.next());
        assertEquals("C", iter1.next());
        assertEquals("D", iter1.next());
        assertFalse(iter1.hasNext()); Iterator<String> iter2 = graph.breadthFirstIterator("B");
        assertEquals("B", iter2.next()); assertEquals("A", iter2.next());
        assertEquals("C", iter2.next()); assertEquals("D", iter2.next());
     ...
     public void testIterator2() {
        Graph<Integer> graph = setupGraphOfIntegers();
        Iterator<Integer> iter1 = graph.breadthFirstIterator(1);
     ...
     public void testDFSStrings() {
        ...
        assertEquals("D", iter1.next());
        assertEquals("B", iter1.next());
        assertFalse(iter1.hasNext()); Iterator<String> iter2 = graph.depthFirstIterator("B");
     ...
     public void testDFSIntegers() {
        ...
        assertEquals(1, iter1.next());
        assertEquals(3, iter1.next());
        assertFalse(iter1.hasNext()); Iterator<Integer> iter2 = graph.depthFirstIterator(3);
        assertEquals(3, iter2.next());
        assertEquals(4, iter2.next());
        assertEquals(6, iter2.next());
        assertEquals(5, iter2.next());
